...
const {t} = useI18n();
const props = defineProps(['plugins', 'pluginsCount', 'adminCountPlugins']);
const form = useForm({});
...
watch(itemsPerPage, (newVal) => {
    axios.put(route('settings.updateAdminCountPlugins'), { value: newVal.toString() })
        .then(response => {
...
const deleteSetting = () => {
    if (pluginToDeleteId.value !== null) {
        form.delete(route('plugins.destroy', pluginToDeleteId.value), {
...
const toggleActivity = (plugin) => {
    ...
    axios.put(route('plugins.updateActivity', plugin.id), {activity: newActivity})
        .then(response => {
        ...
        .catch(error => {
            console.error("Ошибка при обновлении активности модуля:", error.response?.data || error.message);
        });
};
        ...
        <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-12xl mx-auto">
            <div class="p-4 bg-slate-50 dark:bg-slate-700 border border-blue-400 dark:border-blue-200
                ...
                <div class="sm:flex sm:justify-between sm:items-center mb-2">
                    <DefaultButton :href="route('plugins.create')">
                        <template #icon>
