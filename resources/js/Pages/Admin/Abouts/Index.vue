...
import SortSelect from '@/Components/Admin/About/Sort/SortSelect.vue';
import DefaultButton from "@/Components/Admin/Buttons/DefaultButton.vue";
import SectionTable from '@/Components/Admin/About/Table/SectionTable.vue';
...
const props = defineProps(['sections', 'sectionsCount']);
...
const deleteSection = () => {
    if (sectionToDeleteId.value !== null) {
        form.delete(route('abouts.destroy', sectionToDeleteId.value), {
...
const toggleActivity = (section) => {
    ...
    axios.put(route('abouts.updateActivity', section.id), { activity: newActivity })
        .then(response => {
            section.activity = newActivity;
            if (response.data.reload) {
                window.location.reload(); // Перезагрузка страницы при наличии флага
            }
        })
        .catch(error => {
            console.error("Ошибка при обновлении активности секции:", error.response?.data || error.message);
        });
};

// Клонирование рубрики
const cloneSection = (section) => {
    axios.post(route('abouts.clone', section.id))
        .then(response => {
            if (response.data.reload) {
        ...
        .catch(error => {
            console.error("Ошибка клонирования секции:", error);
        });
};
...
const recalculateSort = (event) => {
    paginatedSections.value.forEach((section, index) => {
        ...
        axios.put(route('abouts.updateSort', section.id), {sort: section.sort})
            .then(
...
// Функция массового переключения активности
const bulkToggleActivity = (newActivity) => {
    const requests = selectedSections.value.map(sectionId => {
        return axios.put(route('abouts.updateActivity', sectionId), { activity: newActivity });
    });

    Promise.all(requests)
        .then(responses => {
            const reloadNeeded = responses.some(response => response.data.reload);
            if (reloadNeeded) {
                window.location.reload(); // Перезагрузка страницы при наличии флага в ответах
            }
        })
        .catch(error => {
            console.error("Ошибка при массовом переключении активности:", error.response?.data || error.message);
        });
};

const bulkDelete = () => {
    axios.delete(route('abouts.bulkDestroy'), {data: {ids: selectedSections.value}})
        .then(response => {
        ...
        <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-12xl mx-auto">
            <div class="p-4 bg-slate-50 dark:bg-slate-700 border border-blue-400 dark:border-blue-200
                ...
                <div class="sm:flex sm:justify-between sm:items-center mb-2">
                    <DefaultButton :href="route('abouts.create')">
                        <template #icon>
