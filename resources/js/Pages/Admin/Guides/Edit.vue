...
const form = useForm({
    _method: 'PUT',
    sort: props.guide.sort ?? 0,
    title: props.guide.title ?? '',
    url: props.guide.url ?? '',
    short: props.guide.short ?? '',
    description: props.guide.description ?? '',
    author: props.guide.author ?? '',
    tags: props.guide.tags ?? '',
    views: props.guide.views ?? '',
    likes: props.guide.likes ?? '',
    image_url: null,
    seo_title: props.guide.seo_title ?? '',
    seo_alt: props.guide.seo_alt ?? '',
    meta_title: props.guide.meta_title ?? '',
    meta_keywords: props.guide.meta_keywords ?? '',
    meta_desc: props.guide.meta_desc ?? '',
    activity: Boolean(props.guide.activity ?? false),
    tutorials: []
});
...
const handleTagsInputFocus = () => {
    const tutorialsTitles = form.tutorials.map(tutorial => tutorial.title).filter(Boolean);
    const tags = [...tutorialsTitles];
...
// Путь к изображению
const imagePreview = ref(props.guide.image_url ? props.guide.image_url : null); // Путь уже полный, без /storage/
...
const submitForm = async () => {
    ...
    form.post(route('guides.update', props.guide.id), {
        errorBag: 'editGuide',
        preserveScroll: true,
        onSuccess: () => {
...
onMounted(() => {
    imagePreview.value = props.guide.image_url ? props.guide.image_url : null;
    form.tutorials = props.guide?.tutorials;
});
...
<template>
    <AdminLayout :title="t('editGuide')">
        <template #header>
            <TitlePage>
                {{ t('editGuide') }}: {{ props.guide.title }}
            </TitlePage>
        </template>
        <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-12xl mx-auto">
            <div class="p-4 bg-slate-50 dark:bg-slate-700
                ...
                <div class="sm:flex sm:justify-between sm:items-center mb-2">
                    <DefaultButton :href="route('guides.index')">
                        <template #icon>
                            <!-- SVG -->
                ...
                <form @submit.prevent="submitForm" enctype="multipart/form-data" class="p-3 w-full">
                    ...
                    <div class="mb-3 flex flex-col items-start">
                        <LabelInput for="tutorials" :value="t('tutorials')" class="mb-1"/>
                        <VueMultiselect v-model="form.tutorials"
                                        :options="tutorials"
                                        :multiple="true"
                    ...
                    <div class="mb-3 flex flex-col items-start">
                        <LabelInput for="title" :value="t('guideTitle')"/>
                        <InputText
                            id="title"
                            type="text"
                            v-model="form.title"
                    ...
                    <div class="flex items-center justify-center mt-4">
                        <DefaultButton :href="route('guides.index')" class="mb-3">
                            <template #icon>
                                <!-- SVG -->
                                <svg class="w-4 h-4 fill-current text-slate-100 shrink-0 mr-2" viewBox="0 0 16 16">
                                    <path d="M4.3 4.5c1.9-1.9 5.1-1.9 7 0 .7.7 1.2 1.7 1.4 2.7l2-.3c-.2-1.5-.9-2.8-1.9-3.8C10.1.4 5.7.4 2.9 3.1L.7.9 0 7.3l6.4-.7-2.1-2.1zM15.6 8.7l-6.4.7 2.1 2.1c-1.9 1.9-5.1 1.9-7 0-.7-.7-1.2-1.7-1.4-2.7l-2 .3c.2 1.5.9 2.8 1.9 3.8 1.4 1.4 3.1 2 4.9 2 1.8 0 3.6-.7 4.9-2l2.2 2.2.8-6.4z"></path>
