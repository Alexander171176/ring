...
const updateForm = () => {
    form.sort = props.page?.sort ?? 0;
    form.title = props.page?.title ?? '';
    form.url = props.page?.url ?? '';
    form.slug = props.page?.slug ?? '';
    form.content = props.page?.content ?? '';
    form.meta_title = props.page?.meta_title ?? '';
    form.meta_keywords = props.page?.meta_keywords ?? '';
    form.meta_desc = props.page?.meta_desc ?? '';
    form.activity = Boolean(props.page?.activity ?? false);
    form.print_in_menu = Boolean(props.page?.print_in_menu ?? false);
    form.parent_id = props.page?.parent_id ?? null;
...
const submitForm = async () => {
    form.transform((data) => ({
        ...
        print_in_menu: data.print_in_menu ? 1 : 0,
    }));
