...
const updateForm = () => {
    ...
    form.option = props.setting?.option ?? '';
    form.value = props.setting?.value ?? '';
    form.constant = props.setting?.constant ?? '';
    form.category = props.setting?.category ?? '';
    form.description = props.setting?.description ?? '';
    form.activity = Boolean(props.setting?.activity ?? false);
...
const submitForm = async () => {
    ...
    form.post(route('parameters.update', props.setting.id), {
        errorBag: 'editSetting',
        preserveScroll: true,
        onSuccess: () => {
...
// Обновление формы при изменении пропсов show и setting
watch(
    () => props.show,
    (newShow) => {
        if (newShow) {
