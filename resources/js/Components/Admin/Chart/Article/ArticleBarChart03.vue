...
const articleChartData = computed(() => {
    ...
    return {
        labels: sortedArticles.value.map(article => `ID: ${article.id}`), // Используем ID статей как метки
        datasets: [
            {
                label: 'Лайки статей',
                data: sortedArticles.value.map(article => article.likes), // Количество лайков
...
onMounted(() => {
    // console.log('ArticleBarChart03 data:', articleChartData.value)
    createChart()
})
