...
const rubricChartData = computed(() => {
    ...
    return {
        labels: sortedRubrics.map(rubric => `${rubric.id}: ${rubric.articles_count}`), // Используем ID рубрик и количество статей как метки
        datasets: [
            {
...
onMounted(() => {
    // console.log('RubricBarChart data:', rubricChartData.value)
    createChart()
})
