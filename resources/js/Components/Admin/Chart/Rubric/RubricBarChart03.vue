...
const rubricChartData = computed(() => {
    const sortedRubrics = [...props.rubrics].sort((a, b) => a.id - b.id); // Сортируем рубрики по id
    const colors = getRandomColorsArray(sortedRubrics.length);
    return {
        labels: sortedRubrics.map(rubric => rubric.title), // Используем title рубрик как метки
        datasets: [
            {
...
onMounted(() => {
    // console.log('RubricBarChart03 data:', rubricChartData.value)
    createChart()
})
