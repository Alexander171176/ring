...
const totalArticles = computed(() => props.rubrics.reduce((acc, rubric) => acc + rubric.articles_count, 0));
...
const rubricChartData = computed(() => {
    ...
    return {
        labels: sortedRubrics.map(rubric => `${rubric.id}: ${rubric.title}`), // Используем ID рубрик и количество статей как метки
        datasets: [
            {
                label: 'Количество статей',
                data: sortedRubrics.map(rubric => rubric.articles_count), // Количество статей
...
onMounted(() => {
    // console.log('RubricBarChart05 data:', rubricChartData.value)
    createChart()
})
