...
const imagePreview = ref(props.article?.image_url ? `/storage/${props.article.image_url}` : null);
...
const updateForm = () => {
    form.sort = props.article?.sort ?? 0;
    ...
    form.views = props.article?.views ?? '';
    form.likes = props.article?.likes ?? '';
    form.seo_title = props.article?.seo_title ?? '';
    form.seo_alt = props.article?.seo_alt ?? '';
    form.meta_title = props.article?.meta_title ?? '';
    form.meta_keywords = props.article?.meta_keywords ?? '';
    form.meta_desc = props.article?.meta_desc ?? '';
    form.activity = Boolean(props.article?.activity ?? false);
    form.rubrics = props.article?.rubrics ?? [];
    imagePreview.value = props.article?.image_url ? `/storage/${props.article.image_url}` : null;

    // console.log("Форма дополнена пропсами:", {
...
watch(
    () => props.rubrics,
    (newRubrics) => {
        // console.log("Обновленные рубрики:", newRubrics);
        form.rubrics = props.article?.rubrics ?? [];
    }
);
</script>
    ...
    <div v-if="show" class="fixed inset-0 flex items-center justify-center z-50 overflow-y-auto custom-scrollbar">
        ...
        <div class="absolute
            ...
            <h2 class="text-center text-2xl font-bold mb-4 text-gray-600 dark:text-slate-100 tracking-wide">
                {{ t('editArticle') }} {{ props.article.id }}
            </h2>
                ...
                <div class="pb-12">
                    ...
                    <div class="mb-3 flex flex-col items-start">
                        ...
                        <div v-if="imagePreview" class="mt-2 w-full">
                            <img :src="imagePreview" alt="Current Image" class="w-full h-fit object-cover">
                        </div>
                    </div>
